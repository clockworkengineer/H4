...
TEST_CASE("Use XML object to parse XML declaration", "[XML][Parse][Declaration]")
{
  ...
  SECTION("Parse XML declaration. ", "[XML]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[0].value.parsed == "1.0");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[1].value.parsed == "UTF-8");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
  }
  ...
  SECTION("Parse version 1.0 encoding == UTF-8 standalone == yes XML declaration. ", "[XML][Parse][Declaration]")
  {
    ...
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[0].value.parsed == "1.0");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[1].value.parsed == "UTF-16");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "yes");
  }
  SECTION("Parse version 1.0 encoding == UTF-8 standalone == yes XML declaration. ", "[XML][Parse][Declaration]")
  {
...
TEST_CASE("Use XML object to parse declaration, root element and check parsed information ", "[XML][Parse][Root]")
{
  ...
  SECTION("Empty root element <contact-info> ", "[XML][Parse]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[1].value.parsed == "UTF-8");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).children.size() == 3);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "contact-info");
  }
  SECTION("Empty root element <AddressBook> ", "[XML][Parse][Root]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).children.size() == 3);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "AddressBook");
  }
  SECTION("Root element <AddressBook> and one child <Address> with contents ", "[XML][Parse][Root]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "AddressBook");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).children.size() == 3);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).getContents() == "    This is some contents    ");
  }
  SECTION("Root element <AddressBook> with multiple sibling <Address> elements and contents ", "[XML][Parse][Root]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "AddressBook");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).children.size() == 7);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).getContents() == "\n    This is some contents 1   ");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).getContents() == "\n    This is some contents 2   ");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][2]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][2]).getContents() == "\n    This is some contents 3   ");
  }
}
...
TEST_CASE("Parse XML elements with attached attributes", "[XML][Parse][Attributes]")
{
  ...
  SECTION("Root attribute with one attached attribute number", "[XML][Parse][[Attributes]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].name == "number");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].value.parsed == "15");
  }
  SECTION("Root attribute with 3 attached attributes number, away, flat", "[XML][Parse][[Attributes]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).children.size() == 3);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "AddressBook");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes.size() == 3);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].name == "number");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].value.parsed == "15");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[1].name == "away");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[1].value.parsed == "yes");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[2].name == "flat");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[2].value.parsed == "no");
  }
...
TEST_CASE("Parse XML elements with comments", "[XML][Parse][Comments]")
{
  ...
  SECTION("Multiple comments inside root element and between its children ", "[XML][Parse][[Comments]")
  {
    ...
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog).attributes[2].value.parsed == "no");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "AddressBook");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).children.size() == 13);
    REQUIRE(XNodeRef<XNodeComment>(*xml.prolog[0].children[1]).comment == "Address one ");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).getContents() == "    This is some contents 1   ");
    REQUIRE(XNodeRef<XNodeComment>(*xml.prolog[0].children[5]).comment == "Address two ");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).getContents() == "    This is some contents 2   ");
    REQUIRE(XNodeRef<XNodeComment>(*xml.prolog[0].children[9]).comment == "Address three ");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][2]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][2]).getContents() == "    This is some contents 3   ");
  }
  ...
  SECTION("A simple comment within element content", "[XML][Parse][[Comments]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).getContents() == "TestTest");
  }
  SECTION("A simple comment within element contents and content remains intact", "[XML][Parse][[Comments]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).getContents() == "Test    Test");
  }
...
TEST_CASE("Parse XML with Unicode character in element names, attributes, comments, character data, and processing instructions. ", "[XML][Parse][Unicode]")
{
  ...
  SECTION("Well-formed XML document including Chinese, Armenian and Cyrillic characters", "[XML][Parse][[Unicode]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).name == "俄语");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes.size() == 1);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].name == "լեզու");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].value.parsed == "ռուսերեն");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).getContents() == "данные");
  }
}
TEST_CASE("Check the pasring of character entities/reference.", "[XML][Parse][Entities]")
{
  ...
  SECTION("Parse entity &amp; in contents area", "[XML][Parse][Entities]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).getContents() == " & ");
  }
  SECTION("Parse entity &quot; in contents area", "[XML][Parse][Entities]")
  {
    xmlString = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
  ...
  SECTION("Parse entity &amp;&quot;&apos;&gt;&lt; in attribute value", "[XML][Parse][Entities]")
  {
    ...
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes.size() == 1);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0]).attributes[0].name == "attr1");
...
TEST_CASE("Check the parsing of XML containing program instructions", "[XML][Parse][PI]")
{
  ...
  SECTION("Parse XML containing PI after declaration and check values", "[XML][Parse][PI]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNode>(*xml.prolog.children[1]).getNodeType() == XNodeType::pi);
    REQUIRE(XNodeRef<XNodePI>(*xml.prolog.children[1]).name == "xml-stylesheet");
    REQUIRE(XNodeRef<XNodePI>(*xml.prolog.children[1]).parameters == "href=\"tutorialspointstyle.css\" type=\"text/css\"");
  }
  SECTION("Parse XML containing PI in root section and check values", "[XML][Parse][PI]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNode>(*xml.prolog[0].children[0]).getNodeType() == XNodeType::pi);
    REQUIRE(XNodeRef<XNodePI>(*xml.prolog[0].children[0]).name == "xml-stylesheet");
    REQUIRE(XNodeRef<XNodePI>(*xml.prolog[0].children[0]).parameters == "href=\"tutorialspointstyle.css\" type=\"text/css\"");
  }
}
TEST_CASE("Parse CDATA SECTION", "[XML][Parse][CDATA]")
{
  ...
  SECTION("Parse XML root containing CDDATA containing a XML tags and check contents", "[XML][Parse][CDATA]")
  {
    ...
    REQUIRE(XNodeRef<XNodeCDATA>(*xml.prolog[0].children[1]).cdata == "<message> Welcome to TutorialsPoint </message>");
  }
...
TEST_CASE("Parse XML with defined namespaces.", "[XML][Parse][Namespace]")
{
  ...
  SECTION("A root document and two namespaces defined in the child two table elements.", "[XML][Parse][Namespace]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "h:table");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces.size() == 1);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].name == "h");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].value.parsed == "http://www.w3.org/TR/html4/");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces.size() == 1);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).name == "f:table");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[0].name == "f");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[0].value.parsed == "https://www.w3schools.com/furniture");
  }
  SECTION("A root document and two namespaces defined in the root element.", "[XML][Parse][Namespace]")
  {
    ...
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "h:table");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces.size() == 2);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].name == "h");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].value.parsed == "http://www.w3.org/TR/html4/");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[1].name == "f");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[1].value.parsed == "https://www.w3schools.com/furniture");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).name == "f:table");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces.size() == 2);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[0].name == "h");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[0].value.parsed == "http://www.w3.org/TR/html4/");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[1].name == "f");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][1]).namespaces[1].value.parsed == "https://www.w3schools.com/furniture");
  }
  ...
  SECTION("A root documement with a default namespace", "[XML][Parse][Namespace]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).name == "tr");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces.size() == 1);
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].name == ":");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog[0][0]).namespaces[0].value.parsed == "http://www.w3.org/TR/html4/");
  }
...
TEST_CASE("Use name for accessing elements", "[XML][Access][ByName]")
{
  ...
  SECTION("Address book access", "[XML][Access][ByName]")
  {
    ...
    XML xml(xmlSource);
    xml.parse();
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"]).name == "AddressBook");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"]["Address"]).name == "Address");
    REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"]["Address"]).getContents() == "Flat A, West Road, Wolverhampton, W1SSX9");
  }
}
...
//                 "</AddressBook>";
//     BufferSource xmlSource(xmlString);
//     XML xml(xmlSource); xml.parse();
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][0]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][0]).isWhiteSpace == true);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][1][0]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][1][0]).isWhiteSpace == false);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][2][0]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][2][0]).isWhiteSpace == true);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][3][0]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][3][0]).isWhiteSpace == false);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][3][1]).getNodeType() == XNodeType::entity);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][3][2]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][3][2]).isWhiteSpace == false);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][4][0]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][4][0]).isWhiteSpace == false);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][4][1]).getNodeType() == XNodeType::cdata);
//     REQUIRE(XNodeRef<XNodeElement>(xml.prolog["AddressBook"][4][2]).getNodeType() == XNodeType::content);
//     REQUIRE(XNodeRef<XNodeContent>(xml.prolog["AddressBook"][4][2]).isWhiteSpace == false);
//   }
// }