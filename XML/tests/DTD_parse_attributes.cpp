...
TEST_CASE("Parse XML DTD with atttributes and check values.", "[XML][DTD][Parse][Attributes]")
{
  ...
  SECTION("XML with internal DTD with attributes to parse and check values", "[XML][DTD][Parse][Attributes]")
  {
    ...
    REQUIRE(xml.getDTDElement("DAY").name == "DAY");
    REQUIRE(xml.getDTDElement("HOLIDAY").name == "HOLIDAY");
    REQUIRE(xml.getDTDElement("DATE").name == "DATE");
    REQUIRE(xml.getDTDElement("PROGRAMSLOT").name == "PROGRAMSLOT");
    REQUIRE(xml.getDTDElement("TIME").name == "TIME");
    REQUIRE(xml.getDTDElement("TITLE").name == "TITLE");
    REQUIRE(xml.getDTDElement("DESCRIPTION").name == "DESCRIPTION");
    REQUIRE(xml.getDTDElement("TVSCHEDULE").attributes.size() == 1);
    REQUIRE(xml.getDTDElement("CHANNEL").attributes.size() == 1);
    REQUIRE(xml.getDTDElement("PROGRAMSLOT").attributes.size() == 1);
    REQUIRE(xml.getDTDElement("TITLE").attributes.size() == 2);
    REQUIRE(xml.getDTDElement("TVSCHEDULE").attributes[0].name == "NAME");
    REQUIRE(xml.getDTDElement("CHANNEL").attributes[0].name == "CHAN");
  ...
  SECTION("XML with internal DTD with elements with multiple attributes to parse and check values", "[XML][DTD][Parse][Attributes]")
  {
    ...
    REQUIRE(xml.getDTDElement("NOTES").content.unparsed == "(#PCDATA)");
  }
}
